...
train %>%
  subset(train$target == 1) -> train_ones
...
train2.lm2 <- glm(target~ps_car_12+ps_car_13+ps_car_14+ps_car_11_cat+
...
1-cv.glm(valid,train2.lm2,K=10,cost=cost)$delta[1]
# 0.1989951 average accuracy score
...
####################################################
#                                                  #
#  NON-LINEAR APPROACH - RANDOM FOREST MODEL   #
#                                                  #
####################################################
...
################################################
# Random Forest using R - TAKE TOO LONG TO RUN #
################################################
# oob.err=double(4)
# test.err=double(4)
...
####################################################
#                                                  #
#   TRY OUT OTHER NON-LINEAR MODELS     #
#                                                  #
####################################################
...
for (i in 1:10){fitted = glm(target~ps_car_12+ps_car_13+ps_car_14+ps_car_11_cat+
                               ps_car_09_cat+ps_car_07_cat+ps_car_04_cat+
                               ...
                               ps_ind_03+ps_ind_02_cat+
                               ps_ind_01, data=train2)
cv.error.10[i] = cv.glm(valid,fitted,K=10)$delta[1]
}
...
# Spline
library(splines)
train2.lm4 <- glm(target~bs(ps_car_12, knots = c(0,0.8,1.2))+ns(ps_car_13,df=3)+ns(ps_car_14, df=3)+ps_car_11_cat+
                    ps_car_09_cat+ps_car_07_cat+ps_car_04_cat+
